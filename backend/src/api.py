...
'''
@TODO uncomment the following line to initialize 
the datbase
!! NOTE THIS WILL DROP ALL RECORDS AND START YOUR 
DB FROM SCRATCH
...
def get_drinks():
    ...
    return jsonify({
        "success": True,
        "drinks": [dr.short() for dr in drinks]
    })
...
def update_drinks(payload, itemId):
    try:
        drink = Drink.query.filter_by(id=itemId).one_or_none()
        if drink is None:
            abort(404)
        drink_json = request.get_json()
...
@app.route('/drinks/<int:itemId>', methods=['DELETE'])
@requires_auth('delete:drinks')
def delete_drinks(payload, itemId):
    try:
        print(itemId)
        drink = Drink.query.filter_by(id=itemId).one_or_none()
        if drink is None:
...
@TODO implement error handlers using the
 @app.errorhandler(error) decorator
    each error handler should return 
    (with approprate messages):
             jsonify({
                    "success": False,
                    "error": 404,
